...
class Leg:
    def __init__(self, hip_servo_id=-1, knee_servo_id=-1, inverted=False):
        ...
        if (not hip_servo_id in SERVO_ZERO_POSITION or not hip_servo_id in HIP_SERVO_MIN_POSITION or
            ...
            print(f"Error: Servo ID {hip_servo_id} or {knee_servo_id} not in constants.")
            return
        ...
        try:
            ...
            print(f"Error initializing servos {hip_servo_id}, {knee_servo_id}: {e}")
            self._hip = None
            self._knee = None
    ...
    def is_idle(self):
        ...
        hip_idle = abs(self._hip.get_physical_angle() - self._hip.get_commanded_angle()) <= SERVO_TOLERANCE
        knee_idle = abs(self._knee.get_physical_angle() - self._knee.get_commanded_angle()) <= SERVO_TOLERANCE
        return hip_idle and knee_idle